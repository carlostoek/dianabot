...
class VIPService:
    ...
    async def redeem_token(self, user_id: int, token: str, channel_id: int, duration_minutes: int = 60):
        async with self.sessionmaker() as session:
            stmt = select(VIPToken).where(VIPToken.token == token)
            result = await session.execute(stmt)
            vip_token = result.scalar_one_or_none()
            if (vip_token is None or
                vip_token.expires_at and vip_token.expires_at < datetime.utcnow() or
                vip_token.used_count >= vip_token.max_uses):
                return None
